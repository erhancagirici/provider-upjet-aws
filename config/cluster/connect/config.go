...
func Configure(p *config.Provider) { //nolint:gocyclo
	...
	p.AddResourceConfigurator("aws_connect_routing_profile", func(r *config.Resource) {
		...
		r.References["default_outbound_queue_id"] = config.Reference{
			...
			Extractor:     `github.com/crossplane/upjet/v2/pkg/resource.ExtractParamPath("queue_id",true)`,
		}

		r.Version = "v1beta2"
		// NOTE: removed the custom conversion for status.atProvider.queueConfigsAssociated
		// as the underlying TF provider merged it with status.atProvider.queueConfigs
	})
