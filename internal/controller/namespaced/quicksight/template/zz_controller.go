...
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	...
	for _, i := range o.Provider.Resources["aws_quicksight_template"].InitializerFns {
		initializers = append(initializers, i(mgr.GetClient()))
	}
	rl := reconciliationpolicy.NewExponentialFailureRateLimiter(time.Second, 60*time.Second)
	eventHandler := handler.NewEventHandler(handler.WithDefaultRateLimiter(rl), handler.WithLogger(o.Logger.WithValues("gvk", v1beta1.Template_GroupVersionKind)))
	...
	opts := []managed.ReconcilerOption{
		managed.WithExternalConnecter(
			tjcontroller.NewTerraformPluginSDKAsyncConnector(mgr.GetClient(), o.OperationTrackerStore, o.SetupFn, o.Provider.Resources["aws_quicksight_template"],
	...
	// register webhooks for the kind v1beta1.Template if they're enabled.
	if o.StartWebhooks {
