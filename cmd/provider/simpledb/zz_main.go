...
func main() {
	...
	clusterProvider, err := clusterconfig.GetProvider(ctx, fwProvider, sdkProvider, false, *skipDefaultTags)
	kingpin.FatalIfError(err, "Cannot initialize the cluster provider configuration")
	namespacedProvider, err := namespacedconfig.GetProvider(ctx, fwProvider, sdkProvider, false, *skipDefaultTags)
	...
	kingpin.FatalIfError(clustercontroller.Setup_simpledb(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
	kingpin.FatalIfError(namespacedcontroller.Setup_simpledb(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
